...
#print(df)
print(df[37:40].to_string())
print(df[37:40]['Year'].iloc[-1])

##########################################
#fig1 = px.scatter(df, x='Year', y=['Training compute (FLOPs)'], log_y=True, hover_name='System', color = 'Domain') #, trendline="expanding", trendline_options=dict(function="max"))
fig1b = px.scatter(df[37::], x='Year', y=['Training compute (FLOPs)'], log_y=True, hover_name='System', trendline="ols", trendline_options=dict(log_y=True)) #, trendline="expanding", trendline_options=dict(function="max"))

#fig1a = px.scatter(df[:37], x='Year', y=['Training compute (FLOPs)'], log_y=True, hover_name='System', trendline="expanding", trendline_options=dict(function="max")) #, color = 'Domain') #, trendline="expanding", trendline_options=dict(function="max"))
#fig1b = px.scatter(df[37::], x='Year', y=['Training compute (FLOPs)'], log_y=True, hover_name='System', trendline="expanding", trendline_options=dict(function="max")) #, color = 'Domain') #, trendline="expanding", trendline_options=dict(function="max"))

eq = px.get_trendline_results(fig1b).iloc[0]["px_fit_results"]
b = eq.params[0]
a = eq.params[1]
print(a)
print(b)
...
'''
future_x, future_y = future_trend(fig1b, df['Year'].iloc[-1], 2025)
...
future_x, future_y = future_trend(fig1,df[:52]['Year'].iloc[-1],2025)
fig1f1 = px.scatter(x=future_x, y=future_y, log_y=True, trendline="ols", trendline_options=dict(log_y=True), color_discrete_sequence=['rgb(239, 85, 59)'])
fig1f1.update_traces(marker_size=.01)
...
fig = go.Figure(data = fig1b.data) # + fig1f1.data)
...
fig.update_layout(
        title = {'text': 'AI Systems - Training Completed (FLOPs)','y':0.9,'x':0.5,'xanchor': 'center','yanchor': 'top'},
        yaxis_title="FLOPs",
